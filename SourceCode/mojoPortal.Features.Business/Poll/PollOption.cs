...
namespace PollFeature.Business
{
    ...
    public class PollOption
    {
        ...
        private void GetPollOption(Guid optionGuid)
        {
            using (IDataReader reader = DBPollOption.GetPollOption(optionGuid))
            {
                if (reader.Read())
                {
                    ...
                    if (reader["IsApprove"] != DBNull.Value)
                    {
                        this.isApprove = Convert.ToBoolean(reader["IsApprove"].ToString());
                    }
                    if (reader["IsPublish"] != DBNull.Value)
                    {
        ...
        public static bool DeleteByPollGuid(Guid pollGuid)
        {
            if (pollGuid == Guid.Empty) return false;

            return DBPollOption.DeleteByPollGuid(pollGuid);
        }
        #endregion
        ...
        private static List<PollOption> LoadListFromReader(IDataReader reader)
        {
            ...
            try
            {
                while (reader.Read())
                {
                    ...
                    if (reader["IsApprove"] != DBNull.Value)
                    {
                        pollOption.isApprove = Convert.ToBoolean(reader["IsApprove"].ToString());
                    }
                    if (reader["IsPublish"] != DBNull.Value)
                    {
                        pollOption.isPublish = Convert.ToBoolean(reader["IsPublish"].ToString());
                    }
        ...
        public static List<PollOption> GetOptionsByPollGuidBy(Guid pollGuid)
        {
            IDataReader reader = DBPollOption.GetPollOptionsBy(pollGuid);
            return LoadListFromReader(reader);
        }
        #endregion

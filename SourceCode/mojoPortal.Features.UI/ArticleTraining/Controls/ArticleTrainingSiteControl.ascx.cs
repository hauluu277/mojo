...
{
    public partial class ArticleTrainingSiteControl : UserControl
    {
        #region Properties
            ...
            if (!string.IsNullOrEmpty(category))
            {
                ...
                foreach (var item in categoryList)
                {
                    if (!string.IsNullOrEmpty(item))
                    {
                        ...
                        lblTitle.Text = string.Format("{0} {1}", Resources.ArticleResources.TrandingLabel, coreCategory.Name);
                        break;
                    }
                }
        ...
        public string GenRow(object categoryName, object categoryUrl, object categoryId)
        {
            ...
            {
                result.Append("<tr>");
                result.Append($"<td class='format-td' rowspan='{articleCount + 1 }'><a href='{SiteRoot}{categoryUrl}' title='{categoryName}'>{categoryName}</a></th>");
                result.Append("</tr>");
            }
            else
            {
                result.Append("<tr>");
                result.Append($"<td class='format-td'><a href='{SiteRoot}{categoryUrl}' title='{categoryName}'>{categoryName}</a></td>");
                result.Append("<td></td>");
                result.Append("</tr>");

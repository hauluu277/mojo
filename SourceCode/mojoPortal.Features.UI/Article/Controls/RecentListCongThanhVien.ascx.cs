...
{
    public partial class RecentListCongThanhVien : UserControl
    {
        #region Properties
        ...
        private void BindArticles()
        {
            ...
            pnlDisplayType_ShowImage.Visible = false;
            pnlDisplayType_HideImage.Visible = false;


            string categories = string.Empty;
            if (!string.IsNullOrEmpty(config.ArticleCategoryConfig))
            {
                categories = string.Join(" ", config.ArticleCategoryConfig.Split('-'));
            }

            if (config.TypeThemeSetting == ArticleTabListTypeConstant.ShowImage)
            {
                var reader = Article.GetPageForEndUserCongThanhVien(siteId, moduleId, pageNumber, config.PageSize, categories, keyword.ConvertToVN(), out totalPages);

                rptArticleType_ShowImage.DataSource = reader;
                rptArticleType_ShowImage.DataBind();
                pnlDisplayType_ShowImage.Visible = true;
            }
            else
            {
                var reader = Article.GetPageForEndUserCongThanhVien(siteId, moduleId, pageNumber, config.PageSize, categories, keyword.ConvertToVN(), out totalPages);

                rptArticleType_HideImage.DataSource = reader;
                rptArticleType_HideImage.DataBind();
                pnlDisplayType_HideImage.Visible = true;
            }
        ...
        protected bool EditRefer(int itemId)
        {
            bool show = false;
            ArticleReference articleRefer = ArticleReference.GetArticleByRootId(itemId);
            if (articleRefer.ItemID != -1)

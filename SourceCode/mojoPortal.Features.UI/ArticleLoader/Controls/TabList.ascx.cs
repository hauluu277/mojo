...
namespace ArticleFeature.UI
{
    public partial class TabLoader : UserControl
    {
        ...
        {
            hplTieuDe.Text = config.TitleOrtherSetting;
            hplTieuDe.NavigateUrl = SiteRoot + config.UrlTitleSetting;


            pnlUseTab4.Visible = false;
            mp_modulecontent.Visible = false;
            pnlUseTab5.Visible = false;
            pnlUseTab6.Visible = false;
            pnlUseTab7.Visible = false;
            pnlUseTab8.Visible = false;
            pnlUseTab9.Visible = false;
            pnlUseTab10.Visible = false;
            ...
            else if (config.TabSelectorSetting == ArticleConstant.Tab_6)
            {
                pnlUseTab6.Visible = true;
                BindTab6();
            }
            else if (config.TabSelectorSetting == ArticleConstant.Tab_7)
            {
                pnlUseTab7.Visible = true;
                BindTab7();
            }
            else if (config.TabSelectorSetting == ArticleConstant.Tab_8)
            {
                pnlUseTab8.Visible = true;
                BindTab8();
            }
            else if (config.TabSelectorSetting == ArticleConstant.Tab_9)
            {
                pnlUseTab9.Visible = true;
                BindTab9();
            }
            else if (config.TabSelectorSetting == ArticleConstant.Tab_10)
            {
                pnlUseTab10.Visible = true;
                BindTab10();
            }
            else
            {
        ...
        private void BindTab10()
        {
            if (!string.IsNullOrEmpty(config.ArticleCategoryConfig))
            {
                var listCategory = config.ArticleCategoryConfig.ToListInt('-');
                if (listCategory != null && listCategory.Count > 0)
                {
                    ...
                    //link category
                    hplcategorytab10.Text = category.Name;
                    hplcategorytab10.NavigateUrl = siteRoot + category.Description;
                    var source = Article.GetArticleTopNew(categories, config.NumberArticleLimit, siteId);
                    if (source != null && source.Count > 0)
                    {
                        rptActicleTab10.DataSource = source.Take(2);
                        rptActicleTab10.DataBind();

                        rptActicleTab10_2.DataSource = source.Skip(2).Take(4);
                        rptActicleTab10_2.DataBind();
                    }
                }
            }
        }
        private void BindTab9()
        {
            if (!string.IsNullOrEmpty(config.ArticleCategoryConfig))
            {
                var listCategory = config.ArticleCategoryConfig.ToListInt('-');
                if (listCategory != null && listCategory.Count > 0)
                {
                    ...
                    var category = new CoreCategory(categoryId);
                    //link category
                    hplcategorytab9.Text = category.Name;
                    hplcategorytab9.NavigateUrl = siteRoot + category.Description;
                    var source = Article.GetArticleTopNew(categories, config.NumberArticleLimit, siteId);
                    if (source != null && source.Count > 0)
                    {
                        rptActicleTab9.DataSource = source;
                        rptActicleTab9.DataBind();
                    }
                }
            }
        }
        private void BindTab8()
        {
            if (!string.IsNullOrEmpty(config.ArticleCategoryConfig))
            {
                var listCategory = config.ArticleCategoryConfig.ToListInt('-');
                if (listCategory != null && listCategory.Count > 0)
                {
                    ...
                    var category = new CoreCategory(categoryId);
                    //link category
                    hplcategorytab8.Text = category.Name;
                    hplcategorytab8.NavigateUrl = siteRoot + category.Description;
                    var source = Article.GetArticleTopNew(categories, config.NumberArticleLimit, siteId);
                    if (source != null && source.Count > 0)
                    {
                        rptActicleTab8.DataSource = source;
                        rptActicleTab8.DataBind();
                    }
                }
            }
        }
        private void BindTab7()
        {
            if (!string.IsNullOrEmpty(config.ArticleCategoryConfig))
            {
                var categories = string.Empty;
                ...
                if (listCategory != null && listCategory.Count > 0)
                {
                    ...
                    var category = new CoreCategory(categoryId);
                    //link category
                    hplcategorytab7.Text = category.Name;
                    hplcategorytab7.NavigateUrl = siteRoot + category.Description;
                    var source = Article.GetArticleTopNew(categories, config.NumberArticleLimit, siteId);
                    if (source != null && source.Count > 0)
                    {
                        rptActicleTab7.DataSource = source;
                        rptActicleTab7.DataBind();
                    }
                }
            }
        }
        private void BindTab6()
        {
            if (!string.IsNullOrEmpty(config.ArticleCategoryConfig))
            {
                var categories = string.Empty;
                ...
                if (listCategory != null && listCategory.Count > 0)
                {
                    ...
                    var category = new CoreCategory(categoryId);
                    //link category
                    hplcategorytab6.Text = category.Name;
                    hplcategorytab6.NavigateUrl = siteRoot + category.Description;
                    var source = Article.GetArticleTopNew(categories, config.NumberArticleLimit, siteId);
                    if (source != null && source.Count > 0)
                    {
                        rptActicleTab6.DataSource = source;
                        rptActicleTab6.DataBind();
                    }
                }
            }
        ...
        private void BindCategoriesForTab()
        {
            ...
            if (listChildrenCategory != null && listChildrenCategory.Count > 0)
            {
                ...
                var categories = string.Empty;
                if (!string.IsNullOrEmpty(listCategories))
                {
                    categories = listCategories.Replace("-", " ").Trim();
                }

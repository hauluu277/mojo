    ...
    public partial class PollChoose : NonCmsBasePage
    {
        ...
        private void PopulateControls()
        {
            ...
            if (currentPoll != null)
                ...
                {
                    using (IDataReader reader = Poll.GetPollByPoollGuid(pollGuid))
                    {
        ...
        protected string GetStartActive(string pollGuid)
        {
            Poll poll = new Poll(moduleId);
            string result = "";
            if (poll.PollGuid == Guid.Parse(pollGuid))
            {
                result= PollResources.VerifiedTilte;

            }
            else
            {
                result= PollResources.ActiveLink;
            }
            return result;
        }
        ...
        private void PopulateLabels()
        {
            ...
            lnkNewPoll.NavigateUrl = siteSettings.SiteRoot + "/Poll/PollEdit.aspx?pageid="
                + pageId.ToString(CultureInfo.InvariantCulture)
                + "&mid=" + moduleId.ToString(CultureInfo.InvariantCulture);
        ...
        private void LoadSettings()
        {
            ...
            pollGuid = WebUtils.ParseGuidFromQueryString("pollGuid", pollGuid);
        ...
        protected bool CheckActivated(bool publish, bool approve, bool active, DateTime dateActive)
        {
            if (publish == true && approve == true && active == true && dateActive > DateTime.Now)
            {
                return true;
            }
            return false;
        }
        protected string pollActivedError(bool approve, bool publish)
        {
            string result = PollResources.Poll;
            if (approve == false)
            {
            ...
            if (publish == false)
            {
                result += ", " + PollResources.PollUnPublished;
            }

            return result;
        }
        protected bool checkPollActiveedError(bool approve, bool publish)
        {
            if (publish == false || approve == false)
            {
                return true;
            }
            return false;
        ...
        protected String GetActiveText(Object oActiveFrom, Object oActiveTo)
        {
            ...
            DateTime timeActive = Convert.ToDateTime(oActiveTo);
            if (timeActive <= DateTime.Now)
            {

                text += "<span style='color:red'> &nbsp;" + PollResources.PollTimeError + "</span>";
            }
            return text;
        }
    }

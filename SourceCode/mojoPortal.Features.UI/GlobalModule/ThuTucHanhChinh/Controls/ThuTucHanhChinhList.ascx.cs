...
{
    public partial class ThuTucHanhChinhList : UserControl
    {
        #region Properties
        ...
        private void BindThuTuc()
        {
            int totalVanBan = 0;
            int totalCount = 0;
            var reader = ThuTuc.GetPage(siteSetting.SiteId, true, linhVucId, mucDoId, capThuTucId, keyword, 1, 20, out totalPages, out totalCount);
            rptArticles.DataSource = reader;
            rptArticles.DataBind();
            lblTotalVanBan.Text = totalCount.ToString();
            ...
            lblTotalVanBan.Text = string.Format("Tổng số thủ tục <span class='red'>{0}</span> ", totalCount);
        }
    ...
    public class treeBO
    {
        ...
        public List<treeBO> children { get; set; }
    }
}
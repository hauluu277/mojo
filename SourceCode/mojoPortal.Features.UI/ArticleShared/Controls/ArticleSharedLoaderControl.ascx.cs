...
{
    public partial class ArticleSharedLoaderControl : UserControl
    {
        #region Properties
        ...
        private void PopulateControls()
        {
            ...
            reader = Article.GetTopCategory(config.TopSelectSetting, categories);
            rptArticle.DataSource = reader;
            rptArticle.DataBind();

            //if (!string.IsNullOrEmpty(config.SiteCategorySetting))
            //{
            //    var category = new CoreCategory(Convert.ToInt32(config.SiteCategorySetting.Split('-')[0]));

<%@ Control Language="C#" AutoEventWireup="false" CodeBehind="EventHotList.ascx.cs" Inherits="EventFeature.UI.EventHotList" %>
    ...
    <portal:InnerWrapperPanel ID="pnlInnerWrap" runat="server" CssClass="panelwrapper SlideList">
        <portal:OuterBodyPanel ID="pnlOuterBody" runat="server">
            <portal:InnerBodyPanel ID="pnlInnerBody" runat="server" CssClass="modulecontent">
                <div class="box_eventhot doc_slide">
                    <div class="container">
                    <div class="header_slide">
                        <h2 class="page-header">
                            <asp:HyperLink ID="hplEvent" runat="server"></asp:HyperLink>
                        </h2>
                    </div>
                                ...
                                <ItemTemplate>
                                   <div class="col-sm-6 event_hot__item event_hot__common">
                                        ...
                                        <asp:Image ID="imghotright" Visible='<%#ShowImage(Eval("ImageUrl").ToString())%>' runat="server" Width="100%" ImageUrl='<%# EventUtils.FormatImageDialog(ConfigurationManager.AppSettings["EventImagesFolder"], Eval("ImageUrl").ToString()) %>' />
                                                    <div class="event-hot-hover-icon">
                                            <a href="<%# ArticleUtils.FormatBlogTitleUrl(SiteRoot, Eval("ItemUrl").ToString(), Convert.ToInt32(Eval("ItemID")), PageId, ModuleId) %>" class="fa fa-info" tabindex="0"></a>
                                        </div>
                                    </asp:HyperLink>
                                            </div>
                                   <div class="event_hot__item__clone__content">
                                        <asp:HyperLink ID="lnkDetail" CssClass="event_title_han_lenght" runat="server" NavigateUrl='<%# FormatBlogTitleUrl(SiteRoot, Eval("ItemUrl").ToString(), Convert.ToInt32(Eval("ItemID")), PageId, ModuleId) %>'>
                                            <%#formatContent(Eval("Title").ToString()) %>
                                    </asp:HyperLink>
                                            <ul>
                                                <li>
                                            <span class="event-date"><%#FormatDateTime(Eval("StartTime"),Eval("EndTime")) %></span></li>
                                       ...
                                       <%-- <asp:HyperLink ID="lnkDetail" runat="server" NavigateUrl='<%# FormatBlogTitleUrl(SiteRoot, Eval("ItemUrl").ToString(), Convert.ToInt32(Eval("ItemID")), PageId, ModuleId) %>'>--%>
                                               <div class="event_hot__last__local">
                                                   <div class="event_hot__last___local__StartDate">
                                                       <span class="event-date"><%#FormatDate(Eval("StartTime")) %></span>
                                                   </div>
                                        ...
                                        <%--</asp:HyperLink>--%>
                                        <asp:HyperLink ID="HyperLink2" CssClass="title_right_han_lenght" runat="server" NavigateUrl='<%# FormatBlogTitleUrl(SiteRoot, Eval("ItemUrl").ToString(), Convert.ToInt32(Eval("ItemID")), PageId, ModuleId) %>'>
                                           <%#Eval("Title") %>
                                        </asp:HyperLink>
                                    </li>
                                </ItemTemplate>
...
<script>
    ...
    if (isIE == true) {
        $(document).ready(function() 
            { $(".event_title_han_lenght").each(function(i){ 
        ...
        $(document).ready(function() 
            { $(".title_right_han_lenght").each(function(i){ 
             var len=$(this).text().trim().length; 
             if(len>88) 
             { 
              $(this).text($(this).text().substr(0,88)+'...'); 
             } 
            }); 
        }); 
    }

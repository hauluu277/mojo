                                ...
                                <ItemTemplate>
                                    <div class='<%# "item-wrapper tooltipable" + ModuleId %>'>
                                        <asp:HyperLink ID="hplOtherTitle" runat="server" title="" ToolTip='<%# EventUtils.FormatTooltip(Eval("Title").ToString(), Eval("Description").ToString(), Config) %>'
                                            CssClass="link" Text='<%# Eval("Title") %>' NavigateUrl='<%# EventUtils.FormatBlogTitleUrl(SiteRoot, Eval("ItemUrl").ToString(), Convert.ToInt32(Eval("ItemID")), PageId, ModuleId) %>' />
                                    </div>
                                </ItemTemplate>
                            </asp:TemplateField>
                        </Columns>

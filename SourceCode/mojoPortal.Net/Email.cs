...
namespace mojoPortal.Net
{
    ...
    public static class Email
    {
        ...
        public static bool SendEmailSingle(string to, string fullName, string body, string subject)
        {
            if (ConfigurationManager.AppSettings["ENABLESENDMAIL"] == "1")
            {
                //   <add key="G_MailFrom" value="nguyenhung28996@gmail.com" />
                //<add key="G_MailAlias" value="vanphong.sobentre@moet.edu.vn" />
                //<add key="G_MailHost" value="smtp.gmail.com" />
                //<add key="G_MailPort" value="587" />
                //<add key="G_MailUserName" value="nguyenhung28996@gmail.com" />
                //<add key="G_MailPassword" value="ycbnwdqvudvowxsc" />
                //string ACCOUNT_MAIL = ConfigurationManager.AppSettings["ACCOUNT_MAIL"];
                //string SMTP_MAIL = ConfigurationManager.AppSettings["SMTP_Mail"];
                //string PASS_MAIL = ConfigurationManager.AppSettings["PASS_MAIL"];

                string ACCOUNT_MAIL = ConfigurationManager.AppSettings["G_MailUserName"];
                string SMTP_MAIL = ConfigurationManager.AppSettings["G_MailHost"];
                string PASS_MAIL = ConfigurationManager.AppSettings["G_MailPassword"];
                string portal = ConfigurationManager.AppSettings["G_MailPort"];
                try
                {
                    int PORT_MAIL = Convert.ToInt32(portal);
                    string from = ACCOUNT_MAIL;

                    MailMessage msg = new MailMessage(ACCOUNT_MAIL, to);
                    ...
                    NetworkCredential networkCredential = new NetworkCredential(ACCOUNT_MAIL, PASS_MAIL);
                    ...
                    log.Info($"Gui email cho tai khoan: {to} thanh cong");
                    return true;
                }
                catch (SmtpFailedRecipientException ex)
                {
                    log.Error($"Loi gui email cho tai khoan: {to} - {ex.Message}", ex);
                    return false;
                }
                catch (Exception ex)
                {
                    log.Error($"Exception at SendEmailAsync {ex.Message}", ex);
                    return false;
                }
            }
            return true;

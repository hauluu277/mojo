...
namespace mojoPortal.Web.AdminUI
{
    ...
    public partial class PageProperties : NonCmsBasePage
    {
        ...
        protected override void OnPreInit(EventArgs e)
        {
            ...
            lbCategory.ViewStateMode = ViewStateMode.Enabled;
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            if (pageId > -1)
            {
                divUseDefaultPage.Visible = false;
        ...
        private void PopulateCategories()
        {
            ...
            List<CoreCategory> roots = CoreCategory.GetRootByCat(siteSettings.SiteId, siteSettings.ArticleCategory);
        ...
        {
            cbk_RequestLogin.Checked = pageSettings.IS_Request_Login;
            ...
            if (pageId > -1)
            {
                ...
                //}
                //set selected value for category config
                var list_category_config = pageSettings.CategoryConfig.Split('-');
        ...
        private void Apply_Click(Object sender, EventArgs e)
        {
            ...
            string catConfig = string.Empty;
            if (lbCategory.Items.Count > 0)
            {
                foreach (ListItem listitem in lbCategory.Items)
                {
                    if (listitem.Selected)
                        catConfig += listitem.Value + "-";
                }
            }
            pageSettings.CategoryConfig = catConfig;
            if (SavePageData())
            {
                //save default module for page with current site
                if (pageNewlyCreated && chkUseDefault.Checked)
                {
                    List<DefaultModulePage> listDefaultModules = new List<DefaultModulePage>();
                    using (IDataReader reader = DefaultModulePage.GetDefaultModuleInPage4Site(siteSettings.SiteId))
                    {
                        while (reader.Read())
                        {
                            ...
                            m.ModuleTitle = reader["DefautlTitle"].ToString() + " - " + txtPageName.Text;
                            m.ModuleOrder = Convert.ToInt32(reader["ModuleOrder"]);
                            listDefaultModules.Add(m);
                        }
                    }

                    if (listDefaultModules != null && listDefaultModules.Count > 0)
                    {
                        foreach (var dm in listDefaultModules)
                        {
                            int moduleDefID = dm.ModuleId;
                            ...
                            m.PageId = pageId;
                            m.ModuleTitle = dm.ModuleTitle;
                            m.PaneName = dm.PaneName;
        ...
        void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            FriendlyUrl.DeleteByPageGuid(pageSettings.PageGuid, siteSettings.SiteId, pageSettings.Url);
        ...
        private bool SavePageData()
        {
            ...
            pageSettings.IS_Request_Login = cbk_RequestLogin.Checked;

...
namespace mojoPortal.Web.AdminUI
{
    public partial class ManageUsers : NonCmsBasePage
    {
        ...
        override protected void OnInit(EventArgs e)
        {
            ...
            //btnUpdateSiteUser.Click += new EventHandler(BtnUpdateSiteUser_Click);
        ...
        protected void BtnUpdateSiteUser_Click(object sender, EventArgs e)
        {
            var cc = new List<string>();
            foreach (ListItem item in lboxSiteUse.Items)
            {
                if (item.Selected)
                {
                    cc.Add(item.Value);
                ...
                siteUser.SiteManager = string.Join(",", cc.ToArray());
                siteUser.Save();
            }
            //System.Web.UI.ScriptManager.RegisterStartupScript(pnlUpdateSiteUse, pnlUpdateSiteUse.GetType(), "ReloadTag", "ReloadTag()", true);
        ...
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            PopulateLabels();
            pnlRolePortal.Visible = false;
            if (!IsPostBack)
            {
                if (siteSettings.SiteId == 1)
                {
                    pnlRolePortal.Visible = true;
                    LoadSiteUse();
                }
                PopulateControls();
            }
        ...
        private void LoadSiteUse()
        {
            var lstSite = SiteSettings.GetListAllSite();
            lboxSiteUse.DataValueField = "SiteID";
            lboxSiteUse.DataTextField = "SiteName";
            lboxSiteUse.DataSource = lstSite;
            lboxSiteUse.DataBind();
        }
        ...
        private void PopulateControls()
        {
            ...
            ddlDept.DataValueField = "ItemID";
            ddlDept.DataTextField = "Name";
            ddlDept.DataSource = lstCoQuan;
            ddlDept.DataBind();
            ddlDept.Items.Insert(0, new ListItem("--Chọn--", "0"));
            ...
            {
                if (siteUser.DepartmentId.HasValue)
                {
                    ddlDept.SelectedValue = siteUser.DepartmentId.ToString();
                }
                if (!string.IsNullOrEmpty(siteUser.SiteManager))
                {
                    var lstSelected = siteUser.SiteManager.Split(',');
                    foreach (ListItem item in lboxSiteUse.Items)
                    {
                        foreach (var selected in lstSelected)
                        {
        ...
        private void btnResetPassword_Click(object sender, EventArgs e)
        {
            if ((siteUser != null) && (this.userID > -1))
            {
                ...
                var newPw = CreateRandomPassword(10);

                user.PasswordResetGuid = Guid.Empty;
                mojoMembershipProvider m = Membership.Provider as mojoMembershipProvider;
                user.Password = m.EncodePassword(siteSettings, user, newPw);
                user.Save();
                user.UpdateLastPasswordChangeTime();

                Notification.SendNewPassword(
                    SiteUtils.GetSmtpSettings(),
                    ResourceHelper.GetMessageTemplate("GuiPassword.config"),
        ...
        private void UpdateUser()
        {
            ...
            {
                var siteLogin = siteSettings.SiteId;
                if (WebConfigSettings.UseRelatedSiteMode)
                {
                    siteLogin = WebConfigSettings.RelatedSiteID;
                }
                if (SiteUser.EmailExistsInDB(siteLogin, siteUser.UserId, txtEmail.Text))
                {
            ...
            if (siteUser.LoginName != txtLoginName.Text)
            {
                var siteLogin = siteSettings.SiteId;
                if (WebConfigSettings.UseRelatedSiteMode)
                {
                ...
                if (SiteUser.LoginExistsInDB(siteLogin, txtLoginName.Text))
                {
            ...
            {
                siteUser.DepartmentId = ddlDept.SelectedValue.ToIntOrZero();
            }
        ...
        private void CreateUser()
        {
            ...
            if (SiteUser.EmailExistsInDB(siteLogin, txtEmail.Text))
            {
                lblErrorMessage.Text = Resource.DuplicateEmailMessage;
                return;
            }
            ...
            if (!string.IsNullOrEmpty(ddlDept.SelectedValue))
            {
                user.DepartmentId = ddlDept.SelectedValue.ToIntOrZero();
            }
            ...
            user.SiteSync = siteSettings.SiteId;
            user.SiteManager = siteSettings.SiteId.ToString();
        ...
        private void PopulateLabels()
        {
            ...
            btnResetPassword.Text = Resource.SendNewPassword;
            btnResetPassword.ToolTip = Resource.SendNewPassword;
            SiteUtils.SetButtonAccessKey(btnResetPassword, AccessKeys.ManageUsersResetPasswordButtonAccessKey);
            UIHelper.AddConfirmationDialog(btnResetPassword, Resource.SendNewPasswordConfirm);

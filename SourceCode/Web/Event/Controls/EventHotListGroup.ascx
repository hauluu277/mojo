<%@ Control Language="C#" AutoEventWireup="false" CodeBehind="EventHotListGroup.ascx.cs" Inherits="EventFeature.UI.EventHotListGroup" %>
        ...
        <portal:OuterBodyPanel ID="pnlOuterBody" runat="server">
            <portal:InnerBodyPanel ID="pnlInnerBody" runat="server" CssClass="modulecontent">
               <div class="training_box_eventhot doc_slide">
                    <div class="container">
                    <div class="group_header_slide">
                        <h2 class="page-header">
                            <a href="/su-kien" title="Sự kiện">Sự kiện</a>
                        </h2>
                    </div>
                    <div class="scroll-event-group">
                        <div id="event_hot_group">
                            <asp:Repeater ID="rptEventHot" runat="server">
                                <ItemTemplate>
                                   <div class="event_hot_group__item event_hot_group__common">
                                       <div class="event_hot_group_img">
                                        ...
                                        <asp:Image ID="imghotright" Visible='<%#ShowImage(Eval("ImageUrl").ToString())%>' runat="server" ImageUrl='<%# EventUtils.FormatImageDialog(ConfigurationManager.AppSettings["EventImagesFolder"], Eval("ImageUrl").ToString()) %>' />
                                    </asp:HyperLink>
                                       </div>
                                   <div class="event_hot_group__item__clone__content">
                                        <asp:HyperLink ID="lnkDetail" runat="server" NavigateUrl='<%# FormatBlogTitleUrl(SiteRoot, Eval("ItemUrl").ToString(), Convert.ToInt32(Eval("ItemID")), PageId, ModuleId) %>'>
                                            ...
                                            <ul>
                                                <li>
                                            <p class="event-date"><%#FormatDateTime(Eval("StartDate"),Eval("EndDate")) %></p></li>
                                                <li><p> <%#Eval("Location") %></p></li>
                                            </ul>
                                   </div>
                                   </div>
                                </ItemTemplate>

...
//            if (StringHelper.IsCaseInsensitiveMatch(targetUrl, ".aspx")) return;
//            if (StringHelper.IsCaseInsensitiveMatch(targetUrl, "pagecategory.aspx")) return;
//            if (StringHelper.IsCaseInsensitiveMatch(targetUrl, "pagelinhvuc.aspx")) return;
//            if (!string.IsNullOrEmpty(targetUrl) && targetUrl.ToLower().Equals("admin"))
//            {
//                app.Context.Response.Redirect("/Admin/AdminMenu.aspx"); return;
//            }
//            //if (!string.IsNullOrEmpty(targetUrl) && targetUrl.ToLower().Contains("/personal/"))
//            //{
...
//            //dành cho các phòng chức năng của sở
//            var listCacPhongChucNang = System.Configuration.ConfigurationManager.AppSettings["DS-PHONG-CHUC-NANG"].Split(',').ToList();
//            if (!string.IsNullOrEmpty(targetUrl) && listCacPhongChucNang.Contains(targetUrl.Trim().ToLower()))
//            {
//                var categoryArticle = CoreCategory.GetUrlForArticle(siteSettings.SiteId, "/" + targetUrl);
//                if (categoryArticle != null && categoryArticle.ItemID > 0)
//                {
//                    var queryString = "?cat=" + categoryArticle.ItemID + "&pageid=7412";
//                    var pageName = "/pagecategory.aspx";
//                    DoRewrite(app, queryString, pageName, setClientFilePath);
//                    return;
...
//            {
//                if (!string.IsNullOrEmpty(targetUrl) && !targetUrl.Contains(".aspx"))
//                {

//                    //if (WebConfigSettings.UseFoldersInsteadOfHostnamesForMultipleSites)
//                    //{
//                    //    requestPath = requestPath.Replace("/" + siteSettings.SiteFolderName,string.Empty);
//                    //}
...
//                    //dành cho lĩnh vực thống kê
//                    var category = CoreCategory.GetUrlForLinhVuc(siteSettings.SiteId, "/" + targetUrl);
//                    if (category != null && category.ItemID > 0)
//                    {
//                        var queryString = "?cat=" + category.ItemID;
//                        var pageName = "/pagelinhvuc.aspx";
//                        DoRewrite(app, queryString, pageName, setClientFilePath);
//                        return;
...
//                    {
//                        string input = targetUrl.ToLower();
//                        string pattern = "area";
//                        var requestUrl = app.Request.RawUrl;
//                        MatchCollection matches = Regex.Matches(input, pattern);
//                        if (matches.Count > 0)
//                        {
//                            return;
//                        }
//                        //&&(!targetUrl.Equals("bundles/MsAjaxJs")
//                        if ((targetUrl.Length > 1))
//                        {
//                            var requestLower = requestUrl.ToLower();

//                            if (!string.IsNullOrEmpty(requestLower) && requestLower.Contains("/fileservice?t="))
//                            {
//                                return;
//                            }

//                            if (!string.IsNullOrEmpty(requestLower) && requestLower.Contains("/fileservice/fileupload?t="))
//                            {
//                                return;
//                            }

//                            if (!string.IsNullOrEmpty(requestLower) && requestLower.Contains("/filemanager"))
//                            {
//                                return;
//                            }

//                            if (!string.IsNullOrEmpty(requestLower) && requestLower.Contains("/fileservice?action="))
//                            {
//                                return;
//                            }
    ...
    public class UrlRewriter : IHttpModule
    {
        ...
        private static void DoRewrite(HttpApplication app, string queryStringToUse = "", string realPageName = "", bool setClientFilePath = true)
        {
            ...
            if ((SiteUtils.UsingIntegratedPipeline()) && (WebConfigSettings.UseTransferRequestForUrlReWriting))
            {
                ...
                //app.Context.Server.TransferRequest(realPageName + queryStringToUse, true);
                realPageName = realPageName.Replace("//", "/");
                app.Context.Response.Redirect(realPageName.Replace("~", string.Empty) + queryStringToUse);

...
namespace mojoPortal.Web
{
    ...
    public class mojoMembershipProvider : MembershipProvider
    {
            ...
            out MembershipCreateStatus status)
        {
            ...
            siteUser.SiteManager = siteSettings.SiteId.ToString();
            ...
            if (created)
            {
                if (siteSettings.UseSecureRegistration)
                {
                    ...
                    Notification.SendRegistrationConfirmationLink(
                        ...
                        domain + "/ConfirmRegistration.aspx?ticket=" +
        ...
        public override bool ValidateUser(string userName, string password)
        {
            ...
            //    }
            //}
            var currentSite = CacheHelper.GetCurrentSiteSettings();

            SiteUser siteUser = GetSiteUser(currentSite, userName);
            ...
            {
                //Kiểm tra nếu sử dụng authentication AD 
                //thì không cần kiểm tra mật khẩu account trên cổng
                var loginResult = SessionManager.GetValue("AuthenticationADSuccess");
                if (loginResult != null && bool.Parse(loginResult.ToString()))
                {
                    result = true;
                }
                else

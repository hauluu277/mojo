    ...
    public class SiteTitle : WebControl
    {
        ...
        private void DoRender(HtmlTextWriter writer)
        {
            ...
            if (useLink)
            {
                ...
                if (titleText.Length > 0 && titleText.Length >= 35)
                {
                    titleMarkup = String.Format(CultureInfo.InvariantCulture, "<a class='siteheading' style='font-size: 32px;' href='{0}/Default.aspx'>{1}</a>",
                              Page.ResolveUrl(urlToUse), titleText);
                }
                else
                {
                    titleMarkup = String.Format(CultureInfo.InvariantCulture, "<a class='siteheading' href='{0}/Default.aspx'>{1}</a>",
                                                Page.ResolveUrl(urlToUse), titleText);
                }
            }
            else

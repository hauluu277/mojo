...
namespace mojoPortal.Web.UI
{
    ...
    public partial class CommentsWidget : UserControl, IRefreshAfterPostback, IUpdateCommentStats
    {
        ...
        private bool userCanApprove = false;

        public bool UserCanApprove
        {
            get { return userCanApprove; }
            set { userCanApprove = value; }
        }
        ...
        private string leftPanelCssClass = "theactualdarncomment";
        [Themeable(true)]
        ...
        private string rightPanelCssClass = "commentuserinfo";
        [Themeable(true)]
        ...
        private string commentinfoCssClass = "commentinfo-item";
        [Themeable(true)]
        public string CommentinfoCssClass
        {
            get => commentinfoCssClass;
            set => commentinfoCssClass = value;
        }
        ...
        private void BindComments()
        {
            ...
            var listComment = GetComments();
            rptComments.DataSource = listComment;
            rptComments.DataBind();
            if (listComment.Count == 0)
            {
                pnlInnerPanel.Visible = false;
                commentListHeading.Visible = false;
            }
        ...
        private List<Comment> GetComments()
        {
            //if (sortDescending)
            //{
            return repository.GetByContentDesc(contentGuid, Common.AccessManageComment());
            //}
            //else
            //{
        ...
        protected void rptComments_ItemCommand(object source, RepeaterCommandEventArgs e)
        {
            ...
            {
                case "EditComment":
                    var editcomment = repository.Fetch(new Guid(e.CommandArgument.ToString()));
                    if (editcomment != null)
                    {
                        isNotify = false;
                        hdfCommentGuid.Value = editcomment.Guid.ToString();
                        //txtComment.Text = editcomment.UserComment;
                        edComment.Text = editcomment.UserComment;
                        if (editcomment.UserId > 0)
                        {
                            lblCreateDate.Text = string.Format("{0:dd/MM/yyyy HH:mm}", editcomment.CreatedUtc);
                            lblEmail.Text = editcomment.UserEmail;
                            lblHoTen.Text = editcomment.UserName;
                        }
                        else
                        {
                            lblCreateDate.Text = string.Format("{0:dd/MM/yyyy HH:mm}", editcomment.CreatedUtc);
                            lblEmail.Text = editcomment.AuthorEmail;
                            lblHoTen.Text = editcomment.PostAuthor;
                        }
                        script = "showCommentModel();";
                    }
                    else
                    {
                ...
                case "DeleteComment":
                    ...
                    script = "NotifySuccess('Xóa bình luận thành công!')";
                    break;
                ...
                case "ApproveComment":
                    ...
                    if (c != null)
                    {
                        ...
                        repository.Save(c);
                        script = "NotifySuccess('Xuất bản bình luận thành công!')";
                    }
                    break;
                case "UnApproveComment":
                    Comment comment = repository.Fetch(new Guid(e.CommandArgument.ToString()));
                    if (comment != null)
                    {
                        comment.ModerationStatus = Comment.ModerationRejected;
                        repository.Save(comment);
                        script = "NotifySuccess('Hủy xuất bản bình luận thành công!')";
                    }
                    break;
            ...
            ScriptManager.RegisterStartupScript(pnlReloadComment, GetType(), "CommandComment", script, true);
            if (isNotify)
            {
                BindComments();
        ...
        void rptComments_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            ...
            UIHelper.AddConfirmationDialog(btnDelete, "Dữ liệu xóa sẽ không khôi phục lại được?");
        }
        protected bool CanApprove(object itemId)
        {
            if (Common.AccessManageComment())
            {
                Guid guid = Guid.Parse(itemId.ToString());
                var comment = repository.Fetch(guid);
        ...
        protected bool CanUnApprove(object itemId)
        {
            if (Common.AccessManageComment())
            {
                Guid guid = Guid.Parse(itemId.ToString());
        ...
        private void PopulateLabels()
        {
            ...
            {
                commentListHeading.Text = "Ý kiến bạn đọc";
            }
            ...
            edComment.WebEditor.ToolBar = ToolBar.Custom1;
        ...
        private void LoadSettings()
        {
            ...
            userCanApprove = Common.AccessManageComment();
            SetupCommentSystem();
        }
        ...
        protected void BtnSavecomment_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(hdfCommentGuid.Value))
            {
                ...
                if (comment != null)
                {
                    //comment.UserComment = txtComment.Text;

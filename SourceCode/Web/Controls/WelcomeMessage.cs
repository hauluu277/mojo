    ...
    public partial class WelcomeMessage : WebControl
    {
        ...
        private void DoRender(HtmlTextWriter writer)
        {
            ...
            else
            {
                ...
                if (wrapInProfileLink)
                {
                    //string.Format(format, HttpUtility.HtmlEncode(siteUser.Name))
                    writer.Write("<a class='" + CssClass + "' href='" + SiteUtils.GetNavigationSiteRoot() + "/Secure/UserProfile.aspx" + "'>" + siteUser.Name + "</a>");
                }
                else if (wrapInAnchor)
                {

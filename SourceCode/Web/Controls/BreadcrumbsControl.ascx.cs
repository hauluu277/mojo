...
namespace mojoPortal.Web.UI
{
    ...
    public partial class BreadcrumbsControl : UserControl
    {
        ...
        protected string siteRootRedirect = "/";
        ...
        private void DoRendering()
        {
                ...
                )
            {
                ...
                //return;
                if (Request.Url.AbsolutePath.ToLower().Contains("/article/viewpost.aspx".ToLower())
                    || Request.Url.AbsolutePath.ToLower().Contains("/Article/ViewDetail.aspx".ToLower()))
                {
                    var itemId = WebUtils.ParseInt32FromQueryString("itemid", 0);
                    if (itemId > 0)
                    {
                        var article = new Article(itemId);
                        var listCategory = ArticleCategory.GetList(itemId);
                        if (listCategory != null && listCategory.Count > 0)
                        {
                            pnlBrecumOther.Visible = true;

                            StringBuilder stringBuilder = new StringBuilder();
                            stringBuilder.Append("<ul class='breadcrumb'>");
                            stringBuilder.Append("<li>");
                            stringBuilder.Append("<a href='/' title='" + Resource.HomeLable + "'>" + Resource.HomeLable + "</a>");
                            stringBuilder.Append("</li>");

                            var category = new CoreCategory(listCategory[0].CategoryID.ToIntOrZero());
                            stringBuilder.Append("<li>");
                            stringBuilder.Append("<a href='" + category.Description + "'  class='active' title='" + category.Name + "'>" + category.Name + "</a>");
                            stringBuilder.Append("</li>");

                            stringBuilder.Append("</ul>");
                            literBrecumOther.Text = stringBuilder.ToString();
                        }
                    }
                }

                else if (Request.Url.AbsolutePath.ToLower().Contains("/ArticleTraining/ViewList.aspx".ToLower()))
                {
                    var catId = WebUtils.ParseInt32FromQueryString("catid", 0);
                    if (catId > 0)
                    {
                        var getCoreCategory = new CoreCategory(catId);
                        if (getCoreCategory.ItemID > 0)
                        {
                            pnlBrecumOther.Visible = true;
                            ...
                            stringBuilder.Append("<li>");
                            stringBuilder.Append("<a href='" + Request.Url.AbsolutePath.ToLower() + "'  class='active' title='" + getCoreCategory.Name + "'>" + getCoreCategory.Name + "</a>");
                            stringBuilder.Append("</li>");
                            stringBuilder.Append("</ul>");
                            literBrecumOther.Text = stringBuilder.ToString();
                        }
                ...
                else if (Request.Url.AbsolutePath.ToLower().Contains("/media/ViewListMediaAlbum.aspx".ToLower()))
                {
                    ...
                    stringBuilder.Append("</li>");
                    stringBuilder.Append("<li>");
                    stringBuilder.Append("<a href='" + System.Configuration.ConfigurationManager.AppSettings["LinkTNUE"] + "'  class='active' title='Thư viện ảnh'>Thư viện ảnh</a>");
                    stringBuilder.Append("</li>");
                    stringBuilder.Append("</ul>");
                    literBrecumOther.Text = stringBuilder.ToString();
                }

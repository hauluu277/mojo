    ...
    public class SiteLogo : WebControl
    {
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            else
            {
                ...
                var okee = SiteUtils.IsMobileDevice();
                if (SiteUtils.IsMobileDevice() && overrideImageUrl.Length > 0)
                {
                    imageUrlToUse = Page.ResolveUrl(overrideImageUrl.Replace("../", "/"));
                }

                if (overrideTitle.Length > 0) titleToUse = overrideTitle;
                if (siteSettings.SiteId > 1)
                {
                    var sitePortal = new SiteSettings(1);

                    urlToUse = ConfigurationManager.AppSettings["Domain"];
                    titleToUse = sitePortal.SiteName;
                }

...
namespace mojoPortal.Web.UI
{
    ...
    public class FlexMenu : WebControl
    {
        ...
        private string BuildLiClass(mojoSiteMapNode mojoNode)
        {
            ...
            else if (liCssClass.Length > 0 && !mojoNode.HasVisibleChildren())
            {
                result = liCssClass;
                spacer = " ";
            }
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            //Nếu là trang chủ thì hiển thị icon không hiển thị text
            bool firstHome = true;
            foreach (SiteMapNode childNode in startingNode.ChildNodes)
            {
                ...
                RenderNode(writer, mojoNode, renderDivider, firstHome);
        ...
        private void RenderNode(HtmlTextWriter writer, mojoSiteMapNode mojoNode, bool renderDivider, bool firstHome)
        {
            ...
            if (firstHome)
            {
                string iconHome = "<i class='fa fa-home' aria-hidden='true' ></i>";
                writer.Write(iconHome);
            }
            else
            {
        ...
        private void RenderChildNodes(HtmlTextWriter writer, SiteMapNode node)
        {
            ...
            foreach (SiteMapNode childNode in node.ChildNodes)
            {
                ...
                renderDivider = !String.IsNullOrEmpty(dividerElement) && (nodePos < startingNode.ChildNodes.Count);
                //RenderNode(writer, mojoNode);
                RenderNode(writer, mojoNode, renderDivider, false);

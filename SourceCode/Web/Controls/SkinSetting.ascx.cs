...
namespace mojoPortal.Web.UI
{
    public partial class SkinSetting : UserControl, ISettingControl
    {
        ...
        private void SetupScript()
        {
            ...
            sb.Append("<div class='skin-list'>");
            sb.Append("<ul>");
            foreach (ListItem item in dd.Items)
            {
                if (item.Value.Length == 0) { continue; }
                sb.Append("<li>");
                sb.Append("<a rel='previewitem' class='cboxElement' href='");
                sb.Append(pageUrl + "?skin=");
                sb.Append(item.Value);
                sb.Append("' title='Xem trước giao diện ");
                sb.Append(item.Value);
                sb.Append("'>");
                sb.Append(item.Value);
                sb.Append("</a>");
            ...
            sb.Append("$(\"a[rel='previewitem']\").click(function(e){ ");

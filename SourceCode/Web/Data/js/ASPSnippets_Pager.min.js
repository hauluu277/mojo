function ASPSnippetsPager(a, b) {
    var c = '<li><span style = "cursor:pointer" class="page" page = "{1}">{0}</span><li>';
    var d = "<li class='active'><span>{0}</span><li>";
    var e, f, g;
    var g = 5;
    ...
    if (h > 1) {
        ...
        i = "<nav role='navigation'>";
        i += "<ul class='pagination'>";
        i += "<li class='disabled'>";
        i += "<span>" + b.Page + " " + b.PageIndex + " trong " + totalPage + "</span>";
        i += "</li>";
        if (b.PageIndex > 1) {
            i += c.replace("{0}", "Trang đầu").replace("{1}", "1");
        ...
        if (b.PageIndex < h) {
            i += c.replace("{0}", "»").replace("{1}", b.PageIndex + 1);
            i += c.replace("{0}", "Trang cuối").replace("{1}", h)
        }
    }
    i += "</ul>";
    i += "</nav>";

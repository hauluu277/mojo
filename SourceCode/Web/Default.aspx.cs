...
namespace mojoPortal.Web.UI
{
    ...
    public partial class CmsPage : mojoBasePage
    {
        ...
        private void LoadPage()
        {
            ...
            if (WebConfigSettings.AllowConfigSetting)
            {
                Literal configStyle = new Literal();
                configStyle.ID = "configStyle";
                StringBuilder textStyle = new StringBuilder();
                textStyle.Append("<style type='text/css'>");

                var backgroundColorMenu = CoreCategory.GetByCode(SiteId, ConfigurationManager.AppSettings["BACKGROUND-COLOR-MENU"]);
                if (backgroundColorMenu.ItemID > 0)
                {
                    textStyle.Append("header{background:" + backgroundColorMenu.Color + ";}");
                    textStyle.Append(".ButtonSearch #ctl00_search_button{background-color:" + backgroundColorMenu.Color + ";}");
                }

                var backgroundHeader = CoreCategory.GetByCode(SiteId, ConfigurationManager.AppSettings["BACKGROUND-IMAGE-HEADER"]);
                if (backgroundHeader.ItemID > 0)
                {
                    textStyle.Append(".Banner-School{background-image:url('" + backgroundHeader.PathIMG + "');}");
                }

                textStyle.Append("</style>");
                configStyle.Text = textStyle.ToString();
                Page.Header.Controls.Add(configStyle);
            }
            ...
            foreach (Module module in CurrentPage.Modules)
            {
                ...
                if (StringHelper.IsCaseInsensitiveMatch(module.PaneName, "toppane"))
                {
                    ...
                    {
                        log.Error("Content is assigned to topContent placeholder but it does not exist in layout.master so using center.");
                ...
                if (StringHelper.IsCaseInsensitiveMatch(module.PaneName, "bottompane"))
                {
                    ...
                    {
                        log.Error("Content is assigned to bottomContent placeholder but it does not exist in layout.master so using center.");
                        parent = this.MPContent;
                    }
        ...
        //            AltPane1 = (ContentPlaceHolder)Master.FindControl("topContent");
        //            AltPane2 = (ContentPlaceHolder)Master.FindControl("bottomContent");

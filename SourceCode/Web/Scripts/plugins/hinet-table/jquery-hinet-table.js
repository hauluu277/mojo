...
(function ($) {
    ...
    var initHinetTable = function (options) {
        ...
        */
        ...
        var strHeader = ' <div class="bar__number" style="width:100%;float:left;">';
        ...
        strHeader += '<div style="float:right; padding-right:10px;margin-top: 7px;">';
        ...
        if (isTopBar.length == 0) {
            ...
            } else if ($(".customize-hinet-table").length > 0) {
                $(".customize-hinet-table").html(strHeader);
            } else {
                ...
                for (var col = 0; col < settings.config.length; col++) {
                    ...
                    if (settings.config[col].isCounter == true) {
                        ...
                        if (settings.config[col].content && typeof (settings.config[col].content) === 'function') {
                            ...
                            strContent += "<td " + tdAttr + "='F_" + data.ItemID + "' class='" + tdclass + "'>" + strDatacolum + "</td>";
                        }
                    }
...
function GenPaging(index, total, targetID) {
    var strPage = "";
    strPage += '<li><a href="javascript:ActionPaging(' + 1 + ',' + total + ',\'' + targetID + '\');"><<</a></li>';
    if (index > 3) {
    ...
    strPage += '<li><a href="javascript:ActionPaging(' + total + ',' + total + ',\'' + targetID + '\');">>></a></li>';
...
//            for (var i = 0; i < settings.config.length; i++) {
//                if (settings.config[i].isSort == true) {
//                    hinet_baseElement.find(" table.hinet-table thead tr th").eq(i + 1).addClass("isSort");
//                }
//                if (settings.config[i].nameModel != '') {
//                    hinet_baseElement.find(" table.hinet-table thead tr th").eq(i + 1).attr("data-model", settings.config[i].nameModel);
//                }
//            }
//            //fill data
//            var strRoot = "";
...
//                var data = settings.listItem[item];
//                var strContent = "<tr>";
//                if (settings.primaryKey && data.hasOwnProperty(settings.primaryKey) && settings.attachPrimaryKeyToRow) {
//                    strContent = "<tr data-row-id='" + data[settings.primaryKey] + "'>";
//                }
//                if (settings.showCheckBox) {
...
//                            var datacolum = settings.config[col].content(data);
//                            var strDatacolum = datacolum != null ? datacolum : "";
//                            if (settings.checkUnreadProp && data.hasOwnProperty(settings.checkUnreadProp) && !data[settings.checkUnreadProp]) {
//                                tdclass += " unread"
//                            }

//                            if (settings.colUnreadIcon && settings.colUnreadIcon == settings.config[col].nameModel && !data[settings.checkUnreadProp]) {
//                                strDatacolum = '<img class="img-unread" src="/assets/images/icon_new_vanban.gif" title="Chưa đọc"/>&nbsp;' + strDatacolum;
//                            }
//                            strContent += "<td class='" + tdclass + "'>" + strDatacolum + "</td>";
//                        }
//                    }
...
//                            if (settings.colUnreadIcon && settings.colUnreadIcon == config[col].nameModel && !data[settings.checkUnreadProp]) {
//                                strDatacolum = '<img class="img-unread" src="/assets/images/icon_new_vanban.gif" title="Chưa đọc"/>&nbsp;' + strDatacolum;
//                            }

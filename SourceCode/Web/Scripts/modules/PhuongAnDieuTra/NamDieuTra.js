 ...
 */
function onEdit(id) {
    OpenGlobalModal('get', '/PhuongAnDieuTraArea/NamDieuTra/FormClient', { id: id });
}
...
function onDelete(id, obj) {
    ...
    if (confirm("Xác nhận Xóa ứng dụng '" + name + "'?")) {
        $.ajax({
            type: "post",
            url: "/PhuongAnDieuTraArea/NamDieuTra/Delete",
            data: { id: id },
            success: function (rs) {
                if (rs.Status) {
...
function pagingConfig() {
    ...
    conf = [
        ...
        },
        {
            tdClass: "center",
            isSort: false,
            nameModel: 'ClientCallBack',
            content: function (data) {
                var result = data.ClientCallBack;
                return result;
            }
        },
        {
    ...
    var getData = function (pageIndex, sortQuery, pageSize) {
        $.ajax({
            url: '/PhuongAnDieuTraArea/NamDieuTra/GetData',
            type: 'post',
            cache: false,
            data: {
                "pageIndex": pageIndex,

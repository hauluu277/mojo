...
function pagingConfig() {
    ...
    conf = [
        ...
        },
        {
            ...
            nameModel: 'SiteName',
            content: function (data) {
                var html = `<a href="/${data.UrlSiteMap}/home">${data.SiteName}</a>`;
...

        {
            ...
            content: function (data) {
                ...
                result += '<a href="/Admin/WebBuilder/editwebbuilder.aspx?siteid=' + data.SiteID + '" class="btn-edit hvr-radial-out" data-id="10595" title="Chỉnh sửa"><i class="fa fa-pencil-square fa-lg"></i></a>'
    ...
    var getData = function (pageIndex, sortQuery, pageSize) {
        $.ajax({
            url: '/investigateArea/investigate/GetData',
            type: 'post',
            cache: false,
            data: {
                "pageIndex": pageIndex,
...
function OpenService(siteId) {
    OpenGlobalModal("get", "/investigateArea/investigate/IndexService", { siteId: siteId }, true);
}
...
function detailProduct(id) {
    CallAjaxLoading("get", "/investigateArea/investigate/DetailSanPham", { id: id }, true, function (rs) {
        OpenGlobalModal(rs);
    })
}

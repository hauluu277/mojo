...
namespace mojoPortal.Business
{
    ...
    public class SiteUser
    {
        ...
        private void GetUserByLoginName(string loginName)
        {
            using (IDataReader reader = DBSiteUser.GetUserByLoginName(1, loginName))
            {
                GetUser(reader);
            }
        }
        ...
        public static List<SiteUserObj> GetStaticUser(string ListUser, DateTime? startDate, DateTime? endDate)
        {
            List<SiteUserObj> result = new List<SiteUserObj>();
            using (var reader = DBSiteUser.GetStaticArticleUser(ListUser, startDate, endDate))
            {
                while (reader.Read())
                {
                    SiteUserObj userStatic = new SiteUserObj();
                    userStatic.CountArticleID = Convert.ToInt32(reader["TotalArticle"]);
                    userStatic.Name = reader["Name"].ToString();
                    result.Add(userStatic);
                }
            }
            return result;
        }
        public static List<int> GetAllUser(int SiteId)
        {
            List<int> result = new List<int>();
            using (var reader = DBSiteUser.GetAllUser(SiteId))
            {
                while (reader.Read())
                {
                    result.Add(Convert.ToInt32(reader["UserID"]));
                }
            }
            return result;
        }
        ...
        public SiteUser(SiteSettings settings, string login)
        {
            if (settings != null)
            {
                ...
                {
                    siteID = RelatedSiteID;
                    siteLogin = siteSettings.SiteId;
                }
                GetUser(login, siteLogin);
            }
        }

        public SiteUser(int siteId, string login)
        {
           
                GetUser(siteId, login);
        }
        ...
        private int siteSync = 0;
        private string siteManager = string.Empty;
        private string siteManagerName = string.Empty;
        ...
        /// </summary>
        /// 
        private string aD_MCS = string.Empty;
        private string aD_TenDonViCS { get; set; }
        private string aD_PhongBan { get; set; }
        private string aD_TenPhongBan { get; set; }

        private string chucVu = string.Empty;
        private string maChucVu = string.Empty;

        private bool isVu = false;
        private bool isChiCuc = false;
        private bool isCucTTDL = false;
        ...
        public bool IS_CHICUC
        {
            get { return isChiCuc; }
            set { isChiCuc = value; }
        }

        public bool IsCucTTDL
        {
            get { return isCucTTDL; }
            set { isCucTTDL = value; }
        }
        ...
        public string AD_TenPhongBan
        {
            get { return aD_TenPhongBan; }
            set { aD_TenPhongBan = value; }
        }
        public string AD_PhongBan
        {
            get { return aD_PhongBan; }
            set { aD_PhongBan = value; }
        }

        public string AD_TenDonViCS
        {
            get { return aD_TenDonViCS; }
            set { aD_TenDonViCS = value; }
        }

        public string AD_MCS
        {
            get { return aD_MCS; }
            set { aD_MCS = value; }
        }
        ...
        private int siteLogin = 0;
        public int SiteLogin
        {
            get { return siteLogin; }
            set { siteLogin = value; }
        }
        ...
        private void GetUser(string loginInfo, int siteLogin = 0)
        {
            IDataReader reader;
            ...
            {
                using (reader = DBSiteUser.GetSingleUser(siteID, loginInfo, siteLogin))
                {
                    GetUser(reader);
                }
            }
        ...
        private void GetUser(int siteId, string loginInfo)
        {
            IDataReader reader;
            ...
            {
                using (reader = DBSiteUser.GetSingleUserCustom(siteID, loginInfo, siteLogin))
                {
                    GetUser(reader);
                }
            }
            ...
            {
                using (reader = DBSiteUser.GetSingleUserByLoginNameCustom
                    (siteID,
                    loginInfo,
                    (siteSettings.UseLdapAuth && siteSettings.AllowDbFallbackWithLdap && siteSettings.AllowEmailLoginWithLdapDbFallback),
                    siteLogin)
        ...
        public static SiteUser GetUserById(int userId)
        {
        ...
        private void GetUser(IDataReader reader)
        {
            ...
            if (reader.Read())
            {
                ...
                if (!string.IsNullOrEmpty(reader["IS_VU"].ToString()))
                {
                    isVu = Convert.ToBoolean(reader["IS_VU"]);
                }

                if (!string.IsNullOrEmpty(reader["IS_CHICUC"].ToString()))
                {
                    isChiCuc = Convert.ToBoolean(reader["IS_CHICUC"]);
                }

                if (!string.IsNullOrEmpty(reader["IS_CUCTTDL"].ToString()))
                {
                    isCucTTDL = Convert.ToBoolean(reader["IS_CUCTTDL"]);
                }
                ...
                if (!string.IsNullOrEmpty(reader["SiteSync"].ToString()))
                {
                    siteSync = Convert.ToInt32(reader["SiteSync"]);
                }
                if (!string.IsNullOrEmpty(reader["SiteManager"].ToString()))
                {
                    siteManager = reader["SiteManager"].ToString();
                }

                if (!string.IsNullOrEmpty(reader["AD_MCS"].ToString()))
                {
                    aD_MCS = reader["AD_MCS"].ToString();
                }

                if (!string.IsNullOrEmpty(reader["AD_TenDonViCS"].ToString()))
                {
                    aD_TenDonViCS = reader["AD_TenDonViCS"].ToString();
                }

                if (!string.IsNullOrEmpty(reader["AD_PhongBan"].ToString()))
                {
                    aD_PhongBan = reader["AD_PhongBan"].ToString();
                }

                if (!string.IsNullOrEmpty(reader["AD_TenPhongBan"].ToString()))
                {
                    aD_TenPhongBan = reader["AD_TenPhongBan"].ToString();
                }


                if (!string.IsNullOrEmpty(reader["MaChucVu"].ToString()))
                {
                    maChucVu = reader["MaChucVu"].ToString();
                }


                if (!string.IsNullOrEmpty(reader["ChucVu"].ToString()))
                {
        ...
        public bool DeleteUserFinal()
        {
            SubscriberRepository subscriptions = new SubscriberRepository();
            var site = new SiteSettings(1);
            List<LetterSubscriber> subs = subscriptions.GetListByUser(site.SiteGuid, userGuid);
            foreach (LetterSubscriber subscription in subs)
            {
                subscriptions.Delete(subscription);
                LetterInfo.UpdateSubscriberCount(subscription.LetterInfoGuid);
        ...
        public bool IsInRoles(String semicolonSeparatedRoleNames)
        {
            ...
            foreach (string roleToCheck in semicolonSeparatedRoleNames.Split(new char[] { ';' }))
            {
                ...
                {
                    if (userRole.ToLower().Equals("admins")) return true;
        ...
        public static IDataReader GetByRole(int siteId, string roleName)
        {
            return DBSiteUser.GetByRoleName(siteId, roleName);
        }


        public static List<SiteUser> GetLoadByRole(int siteId, string roleName)
        {
            return LoadUser(DBSiteUser.GetUserByRoleName(siteId, roleName));
        }
        ...
        public static List<SiteUser> GetBySite(int siteId)
        {
            var reader = DBSiteUser.GetUserList(siteId);
            return LoadUser(reader);
        }
        private static List<SiteUser> LoadUser(IDataReader reader)
        {
            List<SiteUser> result = new List<SiteUser>();
            try
            {
                while (reader.Read())
                {
                    SiteUser siteUser = new SiteUser();
                    siteUser.userID = Convert.ToInt32(reader["UserID"], CultureInfo.InvariantCulture);
                    siteUser.siteID = Convert.ToInt32(reader["SiteID"], CultureInfo.InvariantCulture);
                    try
                    {
                        siteUser.totalRevenue = Convert.ToDecimal(reader["TotalRevenue"], CultureInfo.InvariantCulture);
                    }
                    catch (Exception)
                    { }
                    ...
                    siteUser.loginName = reader["LoginName"].ToString();
                    siteUser.email = reader["Email"].ToString();
                    siteUser.password = reader["Pwd"].ToString();
                    siteUser.gender = reader["Gender"].ToString().Trim();

                    if (reader["ProfileApproved"] != DBNull.Value)
                    {
                        siteUser.profileApproved = Convert.ToBoolean(reader["ProfileApproved"]);
                    }

                    if (reader["ApprovedForForums"] != DBNull.Value)
                    {
                        siteUser.approvedForForums = Convert.ToBoolean(reader["ApprovedForForums"]);
                    }
                    if (reader["Trusted"] != DBNull.Value)
                    {
                        siteUser.trusted = Convert.ToBoolean(reader["Trusted"]);
                    }
                    if (reader["DisplayInMemberList"] != DBNull.Value)
                    {
                        siteUser.displayInMemberList = Convert.ToBoolean(reader["DisplayInMemberList"]);
                    }
                    siteUser.webSiteUrl = reader["WebSiteURL"].ToString();
                    siteUser.country = reader["Country"].ToString();
                    siteUser.state = reader["State"].ToString();
                    siteUser.occupation = reader["Occupation"].ToString();
                    siteUser.interests = reader["Interests"].ToString();
                    siteUser.msn = reader["MSN"].ToString();
                    siteUser.yahoo = reader["Yahoo"].ToString();
                    siteUser.aim = reader["AIM"].ToString();
                    siteUser.icq = reader["ICQ"].ToString();
                    siteUser.totalPosts = Convert.ToInt32(reader["TotalPosts"], CultureInfo.InvariantCulture);
                    siteUser.avatarUrl = reader["AvatarUrl"].ToString();

                    siteUser.timeOffsetHours = Convert.ToInt32(reader["TimeOffsetHours"], CultureInfo.InvariantCulture);
                    siteUser.signature = reader["Signature"].ToString();

                    try
                    {
                        siteUser.dateCreated = Convert.ToDateTime(reader["DateCreated"]);
                    }
                    catch (FormatException ex)
                    {
                        if (log.IsErrorEnabled)
                    ...
                    try
                    {
                        siteUser.userGuid = new Guid(reader["UserGuid"].ToString());
                    }
                    catch (FormatException ex)
                    {
                        if (log.IsErrorEnabled)
                    ...
                    siteUser.skin = reader["Skin"].ToString();
                    if (reader["IsDeleted"] != DBNull.Value)
                    {
                        siteUser.isDeleted = Convert.ToBoolean(reader["IsDeleted"]);
                    }
                    if (reader["IsLockedOut"] != DBNull.Value)
                    {
                        siteUser.isLockedOut = Convert.ToBoolean(reader["IsLockedOut"]);
                    }

                    siteUser.loweredEmail = reader["LoweredEmail"].ToString();
                    siteUser.passwordQuestion = reader["PasswordQuestion"].ToString();
                    siteUser.passwordAnswer = reader["PasswordAnswer"].ToString();
                    try
                    {
                        if (reader["LastActivityDate"] != DBNull.Value)
                        {
                            siteUser.lastActivityDate = Convert.ToDateTime(reader["LastActivityDate"]);
                        }
                    }
                    catch (FormatException ex)
                    {
                    ...
                    try
                    {
                        if (reader["LastLoginDate"] != DBNull.Value)
                        {
                            siteUser.lastLoginDate = Convert.ToDateTime(reader["LastLoginDate"]);
                        }
                    }
                    catch (FormatException ex)
                    {
                    ...
                    try
                    {
                        if (reader["LastPasswordChangedDate"] != DBNull.Value)
                        {
                            siteUser.lastPasswordChangedDate = Convert.ToDateTime(reader["LastPasswordChangedDate"]);
                        }
                    }
                    catch (FormatException ex)
                    {
                    ...
                    try
                    {
                        if (reader["LastLockoutDate"] != DBNull.Value)
                        {
                            siteUser.lastLockoutDate = Convert.ToDateTime(reader["LastLockoutDate"]);
                        }
                    }
                    catch (FormatException ex)
                    {
                    ...
                    try
                    {
                        if (reader["FailedPasswordAttemptCount"] != DBNull.Value)
                        {
                            siteUser.failedPasswordAttemptCount =
                                Convert.ToInt32(reader["FailedPasswordAttemptCount"], CultureInfo.InvariantCulture);
                        }
                    }
                    catch (InvalidCastException ex)
                    ...
                    try
                    {
                        if (reader["FailedPwdAttemptWindowStart"] != DBNull.Value)
                        {
                            siteUser.failedPasswordAttemptWindowStart = Convert.ToDateTime(reader["FailedPwdAttemptWindowStart"]);
                        }
                    }
                    catch (FormatException ex)
                    {
                    ...
                    try
                    {
                        if (reader["FailedPwdAnswerAttemptCount"] != DBNull.Value)
                        {
                            siteUser.failedPasswordAnswerAttemptCount =
                                Convert.ToInt32(reader["FailedPwdAnswerAttemptCount"], CultureInfo.InvariantCulture);
                        }
                    }
                    catch (InvalidCastException ex)
                    ...
                    try
                    {
                        if (reader["FailedPwdAnswerWindowStart"] != DBNull.Value)
                        {
                            siteUser.failedPasswordAnswerAttemptWindowStart = Convert.ToDateTime(reader["FailedPwdAnswerWindowStart"]);
                        }
                    }
                    catch (FormatException ex)
                    {
                    ...
                    siteUser.mobilePIN = reader["MobilePIN"].ToString();
                    siteUser.passwordSalt = reader["PasswordSalt"].ToString();
                    siteUser.comment = reader["Comment"].ToString();
                    string confirmGuidString = reader["RegisterConfirmGuid"].ToString();
                    if (confirmGuidString.Length == 36)
                    {
                        siteUser.registerConfirmGuid = new Guid(confirmGuidString);
                    }

                    siteUser.openIDURI = reader["OpenIDURI"].ToString();
                    siteUser.windowsLiveID = reader["WindowsLiveID"].ToString();
                    siteUser.siteGuid = new Guid(reader["SiteGuid"].ToString());
                    siteUser.mustChangePwd = Convert.ToBoolean(reader["MustChangePwd"]);

                    siteUser.emailChangeGuid = new Guid(reader["EmailChangeGuid"].ToString());
                    siteUser.newEmail = reader["NewEmail"].ToString();
                    siteUser.timeZoneId = reader["TimeZoneId"].ToString();
                    siteUser.editorPreference = reader["EditorPreference"].ToString();
                    siteUser.firstName = reader["FirstName"].ToString();
                    siteUser.lastName = reader["LastName"].ToString();

                    siteUser.passwordResetGuid = new Guid(reader["PasswordResetGuid"].ToString());

                    siteUser.rolesChanged = Convert.ToBoolean(reader["RolesChanged"]);

                    siteUser.authorBio = reader["AuthorBio"].ToString();

                    if (reader["DateOfBirth"] != DBNull.Value)
                    {
                        siteUser.dateOfBirth = Convert.ToDateTime(reader["DateOfBirth"]);
                    }

                    siteUser.emailConfirmed = Convert.ToBoolean(reader["EmailConfirmed"]);
                    siteUser.passwordFormat = Convert.ToInt32(reader["PwdFormat"], CultureInfo.InvariantCulture);
                    siteUser.passwordHash = reader["PasswordHash"].ToString();
                    siteUser.securityStamp = reader["SecurityStamp"].ToString();
                    siteUser.phoneNumber = reader["PhoneNumber"].ToString();
                    siteUser.phoneNumberConfirmed = Convert.ToBoolean(reader["PhoneNumberConfirmed"]);
                    siteUser.twoFactorEnabled = Convert.ToBoolean(reader["TwoFactorEnabled"]);

                    if (reader["LockoutEndDateUtc"] != DBNull.Value)
                    {
                        siteUser.lockoutEndDateUtc = Convert.ToDateTime(reader["LockoutEndDateUtc"]);
                    }
                    if (!string.IsNullOrEmpty(reader["SiteManager"].ToString()))
                    {
                        siteUser.siteManager = reader["SiteManager"].ToString();
                    }
                    if (!string.IsNullOrEmpty(reader["SiteSync"].ToString()))
                    {
                        siteUser.siteSync = Convert.ToInt32(reader["SiteSync"]);
                    }


                    if (!string.IsNullOrEmpty(reader["ChucVu"].ToString()))
                    {
                        siteUser.chucVu = reader["ChucVu"].ToString();
                    }


                    if (!string.IsNullOrEmpty(reader["MaChucVu"].ToString()))
                    {
                        siteUser.maChucVu = reader["MaChucVu"].ToString();
                    }
        ...
        public static SiteUser GetByEmail(SiteSettings siteSettings, string email)
        {
            ...
            using (IDataReader reader = DBSiteUser.GetSingleUser(siteId, email, siteSettings.SiteId))
            {
                siteUser.GetUser(reader);
            }
        ...
        public static DataTable GetUserBySite(int siteId)
        {
            DataTable dataTable = new DataTable();
            dataTable.Columns.Add("UserID", typeof(int));
            dataTable.Columns.Add("LoginName", typeof(string));
            dataTable.Columns.Add("Name", typeof(string));
            using (var reader = DBSiteUser.GetUserList(siteId))
            {
                while (reader.Read())
                {
        ...
        public static DataTable GetUserBySiteGuid(int siteId)
        {
            DataTable dataTable = new DataTable();
        ...
        public static int UserCount(int siteId, int siteManager, String userNameBeginsWith)
        {
            if (UseRelatedSiteMode) { siteId = RelatedSiteID; }
            return DBSiteUser.UserCount(siteId, siteManager, userNameBeginsWith);
        }
        ...
        public static List<SiteUser> GetPageSync(
         int siteSync,
         int pageNumber,
         int pageSize,
         ...
         out int totalPages)
        {
            ...
            using (IDataReader reader
                = DBSiteUser.GetUserListPageSync(
                    siteSync, pageNumber, pageSize, userNameBeginsWith, sortMode, out totalPages))
            {
            ...
            out int totalPages)
        {
            ...
            using (IDataReader reader
                = DBSiteUser.GetUserListPage(
                    siteId, siteManage, pageNumber, pageSize, userNameBeginsWith, sortMode, out totalPages))
            {
        ...
        public static List<SiteUser> GetUserSearchPageSync(
        int siteSync,
        int pageNumber,
        int pageSize,
        string searchInput,
        ...
        out int totalPages)
        {
            ...
            using (IDataReader reader = DBSiteUser.GetUserSearchPageSync(
                siteSync,
                pageNumber,
                pageSize,
                searchInput,
        ...
        public static List<SiteUser> GetUserAdminSearchPageSync(
         int siteId,
         int pageNumber,
         int pageSize,
         string searchInput,
         ...
         out int totalPages)
        {
            ...
            using (IDataReader reader = DBSiteUser.GetUserAdminSearchPageSync(
                siteId,
                pageNumber,
                pageSize,
                searchInput,
        ...
        public static List<SiteUser> GetPageLockedUsersSync(
            int siteId,
            int pageNumber,
            int pageSize,
            out int totalPages)
        {
            ...
            using (IDataReader reader = DBSiteUser.GetPageLockedUsersSync(
                siteId,
                pageNumber,
                pageSize,
                out totalPages))
        ...
        public static List<SiteUser> GetNotApprovedUsersSync(
    int siteSync,
    int pageNumber,
    int pageSize,
    out int totalPages)
        {
            ...
            using (IDataReader reader = DBSiteUser.GetPageNotApprovedUsersSync(
                siteSync,
                pageNumber,
                pageSize,
                out totalPages))
            ...
            IDataReader reader)
        {
            ...
            if (!string.IsNullOrEmpty(reader["SiteManager"].ToString()))
            {
                user.siteManager = reader["SiteManager"].ToString();
                user.siteManagerName = SiteSettings.GetSiteName(user.siteManager);
            }
        ...
        public static IDataReader GetUserByEmail(int siteId, string email)
        {
            ...
            return DBSiteUser.GetSingleUser(siteId, email, siteLogin);
        }
        ...
        public static IDataReader GetUserByLoginName(int siteId, String loginName, bool allowEmailFallback)
        {
            ...
            return DBSiteUser.GetSingleUserByLoginName(siteId, loginName, allowEmailFallback, siteLogin);
        }
        ...
        public static bool EmailExistsInDB(int siteId, string email)
        {
            ...
            using (IDataReader r = DBSiteUser.GetSingleUser(siteId, email, siteLogin))
            {
        ...
        public static bool LoginExistsInDB(int siteId, string loginName)
        {
            ...
            using (IDataReader r = DBSiteUser.GetSingleUserByLoginName(siteId, loginName, false, siteLogin))
            {
                while (r.Read()) { found = true; }
            }
        ...
        public static String GetUserNameFromEmail(int siteId, String email)
        {
            ...
            if ((email != null) && (email.Length > 0) && (siteId > 0))
            {
                ...
                using (IDataReader reader = DBSiteUser.GetSingleUser(siteId, email, siteLogin))
                {
                    while (reader.Read())
                    {

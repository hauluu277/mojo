...
namespace mojoPortal.Business
{
    ...
    public class PageSettings : IComparable
    {
        ...
        private bool is_Request_Login = false;

        private string linkRel = string.Empty;

        private string categoryConfig = string.Empty;
        private string pageNameEN = string.Empty;
        private List<PageSettings> listChildren = null;
        ...
        public List<PageSettings> ListChildren
        {
        ...
        public bool IS_Request_Login
        {
            get
            {
                return is_Request_Login;
            }
            set
            {
                is_Request_Login = value;
            }
        }
        ...
        {
            get { return categoryConfig; }
            set { categoryConfig = value; }
        }
        public string PageNameEN
        {
            get { return pageNameEN; }
            set { pageNameEN = value; }
        }
        #endregion
        ...
        private void LoadFromReader(IDataReader reader)
        {
            ...
            if (reader.Read())
            {
                ...
                if (reader["CategoryConfig"] != DBNull.Value)
                {
                    this.categoryConfig = reader["CategoryConfig"].ToString();
                }
                if (reader["PageNameEN"] != DBNull.Value)
                {
                    this.pageNameEN = reader["PageNameEN"].ToString();
                }
                if (reader["IS_Request_Login"] != DBNull.Value)
                {
                    this.is_Request_Login = Convert.ToBoolean(reader["IS_Request_Login"]);
                }
        ...
        public static List<PageSettings> GetListParent(int siteId)
        {
            var reader = DBPageSettings.GetListParent(siteId);
            return LoadShortFromReader(reader);
        }
        public static List<PageSettings> GetListChild(int siteId)
        {
            var reader = DBPageSettings.GetListChild(siteId);
            return LoadShortFromReader(reader);
        }

        public static int GetPageIdByCategory(string categoryId)
        {
            return DBPageSettings.GetPageIdByCategory(categoryId);
        }
        ...
        public static List<PageSettings> GetList(int siteId)
        {
            return LoadStaticFromReader(DBPageSettings.GetPageList(siteId));
        }
        public static List<PageSettings> GetRoot(int siteId)
        {
            return LoadStaticFromReader(DBPageSettings.GetRoot(siteId));
        }


        public static List<PageSettings> LoadStaticFromReader(IDataReader reader)
        {
            List<PageSettings> listPage = new List<PageSettings>();

            while (reader.Read())
            {
                PageSettings pageSetting = new PageSettings();
                pageSetting.pageID = int.Parse(reader["PageID"].ToString());
                pageSetting.parentID = int.Parse(reader["ParentID"].ToString());
                pageSetting.siteID = int.Parse(reader["SiteID"].ToString());
                pageSetting.pageOrder = int.Parse(reader["PageOrder"].ToString());
                pageSetting.pageName = reader["PageName"].ToString();
                pageSetting.pageTitle = reader["PageTitle"].ToString();
                pageSetting.menuImage = reader["MenuImage"].ToString();
                pageSetting.skin = reader["Skin"].ToString();
                pageSetting.authorizedRoles = reader["AuthorizedRoles"].ToString();
                pageSetting.editRoles = reader["EditRoles"].ToString();
                pageSetting.draftEditOnlyRoles = reader["DraftEditRoles"].ToString();
                pageSetting.draftApprovalRoles = reader["DraftApprovalRoles"].ToString();
                pageSetting.createChildPageRoles = reader["CreateChildPageRoles"].ToString();
                pageSetting.createChildDraftRoles = reader["CreateChildDraftRoles"].ToString();

                pageSetting.requireSSL = Convert.ToBoolean(reader["RequireSSL"]);
                pageSetting.allowBrowserCache = Convert.ToBoolean(reader["AllowBrowserCache"]);
                pageSetting.showBreadcrumbs = Convert.ToBoolean(reader["ShowBreadcrumbs"]);
                pageSetting.showChildPageBreadcrumbs = Convert.ToBoolean(reader["ShowChildBreadCrumbs"]);
                pageSetting.useUrl = Convert.ToBoolean(reader["UseUrl"]);
                pageSetting.url = reader["Url"].ToString();
                //if (useUrl)
                //{
                //    unmodifiedUrl = reader["Url"].ToString();
                //}
                ...
                //    unmodifiedUrl = "~/Default.aspx?pageid=" + pageID.ToString(CultureInfo.InvariantCulture);
                //}
                pageSetting.openInNewWindow = Convert.ToBoolean(reader["OpenInNewWindow"]);
                pageSetting.showChildPageMenu = Convert.ToBoolean(reader["ShowChildPageMenu"]);
                pageSetting.hideMainMenu = Convert.ToBoolean(reader["HideMainMenu"]);

                pageSetting.pageMetaKeyWords = reader["PageKeyWords"].ToString();
                pageSetting.pageMetaDescription = reader["PageDescription"].ToString();
                pageSetting.pageMetaEncoding = reader["PageEncoding"].ToString();
                pageSetting.pageMetaAdditional = reader["AdditionalMetaTags"].ToString();
                pageSetting.includeInMenu = Convert.ToBoolean(reader["IncludeInMenu"]);

                string cf = reader["ChangeFrequency"].ToString();
                switch (cf)
                {
                    case "Always":
                        pageSetting.changeFrequency = PageChangeFrequency.Always;
                        break;

                    case "Hourly":
                        pageSetting.changeFrequency = PageChangeFrequency.Hourly;
                        break;

                    case "Daily":
                        pageSetting.changeFrequency = PageChangeFrequency.Daily;
                        break;

                    case "Monthly":
                        pageSetting.changeFrequency = PageChangeFrequency.Monthly;
                        break;

                    case "Yearly":
                        pageSetting.changeFrequency = PageChangeFrequency.Yearly;
                        break;

                    case "Never":
                        pageSetting.changeFrequency = PageChangeFrequency.Never;
                        break;

                    case "Weekly":
                    default:
                        pageSetting.changeFrequency = PageChangeFrequency.Weekly;
                        break;
                ...
                string smp = reader["SiteMapPriority"].ToString().Trim();
                if (smp.Length > 0) pageSetting.siteMapPriority = smp;

                if (reader["LastModifiedUTC"] != DBNull.Value)
                {
                    pageSetting.lastModifiedUTC = Convert.ToDateTime(reader["LastModifiedUTC"]);
                }

                pageSetting.pageGuid = new Guid(reader["PageGuid"].ToString());
                pageSetting.parentGuid = new Guid(reader["ParentGuid"].ToString());
                pageSetting.hideAfterLogin = Convert.ToBoolean(reader["HideAfterLogin"]);
                pageSetting.siteGuid = new Guid(reader["SiteGuid"].ToString());
                pageSetting.compiledMeta = reader["CompiledMeta"].ToString();
                if (reader["CompiledMetaUtc"] != DBNull.Value)
                {
                    pageSetting.compiledMetaUtc = Convert.ToDateTime(reader["CompiledMetaUtc"]);
                }

                pageSetting.includeInSiteMap = Convert.ToBoolean(reader["IncludeInSiteMap"]);
                pageSetting.isClickable = Convert.ToBoolean(reader["IsClickable"]);
                pageSetting.showHomeCrumb = Convert.ToBoolean(reader["ShowHomeCrumb"]);
                pageSetting.isPending = Convert.ToBoolean(reader["IsPending"]);

                pageSetting.includeInSearchMap = Convert.ToBoolean(reader["IncludeInSearchMap"]);
                pageSetting.canonicalOverride = reader["CanonicalOverride"].ToString();
                pageSetting.enableComments = Convert.ToBoolean(reader["EnableComments"]);

                pageSetting.pubTeamId = new Guid(reader["PubTeamId"].ToString());
                pageSetting.includeInChildSiteMap = Convert.ToBoolean(reader["IncludeInChildSiteMap"]);
                pageSetting.expandOnSiteMap = Convert.ToBoolean(reader["ExpandOnSiteMap"]);

                pageSetting.bodyCssClass = reader["BodyCssClass"].ToString();
                pageSetting.menuCssClass = reader["MenuCssClass"].ToString();

                pageSetting.publishMode = Convert.ToInt32(reader["PublishMode"].ToString());

                if (reader["PCreatedUtc"] != DBNull.Value)
                {
                    pageSetting.createdUtc = Convert.ToDateTime(reader["PCreatedUtc"]);
                }
                ...
                if (reader["PCreatedBy"] != DBNull.Value)
                {
                    ...
                    if (pcg.Length == 36)
                    {
                        pageSetting.createdBy = new Guid(pcg);
                    }

                }

                pageSetting.createdFromIp = reader["PCreatedFromIp"].ToString();

                if (reader["PLastModUtc"] != DBNull.Value)
                {
                    pageSetting.lastModUtc = Convert.ToDateTime(reader["PLastModUtc"]);
                }
                ...
                if (reader["PLastModBy"] != DBNull.Value)
                {
                    ...
                    if (pcg.Length == 36)
                    {
                        pageSetting.lastModBy = new Guid(pcg);
                    }

                }

                pageSetting.lastModFromIp = reader["PLastModFromIp"].ToString();

                pageSetting.menuDescription = reader["MenuDesc"].ToString();

                if (reader["PubDateUtc"] != DBNull.Value)
                {
                    pageSetting.pubDateUtc = Convert.ToDateTime(reader["PubDateUtc"]);
                }

                pageSetting.showPageHeading = Convert.ToBoolean(reader["ShowPageHeading"]);
                pageSetting.linkRel = reader["LinkRel"].ToString();
                pageSetting.pageHeading = reader["PageHeading"].ToString();
                if (reader["CategoryConfig"] != DBNull.Value)
                {
                    pageSetting.categoryConfig = reader["CategoryConfig"].ToString();
                }
                if (reader["PageNameEN"] != DBNull.Value)
                {
                    pageSetting.pageNameEN = reader["PageNameEN"].ToString();
                }
                if (reader["IS_Request_Login"] != DBNull.Value)
                {
                    pageSetting.is_Request_Login = Convert.ToBoolean(reader["IS_Request_Login"]);
                }
                listPage.Add(pageSetting);
        ...
        public static List<PageSettings> LoadShortFromReader(IDataReader reader)
        {
            List<PageSettings> listPage = new List<PageSettings>();
        ...
        public static List<int> GetPageListParent(int siteId, int pageId)
        {
            var listId = new List<int>();
            var reader = DBPageSettings.GetPageListParent(siteId, pageId);
            try
            {
                while (reader.Read())
                {
                    listId.Add(Convert.ToInt32(reader["PageID"]));
                }
            }
            finally
            {
    ...
    public class PageBO
    {
        ...
        public static List<PageBO> GetListParent(int siteId)
        {
            var reader = DBPageSettings.GetListParent(siteId);
            return LoadFromReader(reader);
        }
        public static List<PageBO> GetListChild(int siteId)
        {
            var reader = DBPageSettings.GetListChild(siteId);
            return LoadFromReader(reader);
        }

        private static List<PageBO> LoadFromReader(IDataReader reader)
        {
            var listPage = new List<PageBO>();
            try
            {
                while (reader.Read())
                {
                    PageBO page = new PageBO();
                    page.PageID = int.Parse(reader["PageID"].ToString());
                    page.ParentID = int.Parse(reader["ParentID"].ToString());
                    page.SiteID = int.Parse(reader["SiteID"].ToString());
                    page.PageOrder = int.Parse(reader["PageOrder"].ToString());

...
namespace mojoPortal.Business
{
    ...
    public class Module : IComparable
    {
        ...
        public Module(int skinPageModule, bool isSkin)
        {
            GetModuleForSkinPageModule(skinPageModule);
        }
        ...
        private int skinPageModule = -1;

        #endregion

        #region Public Properties

        public int SkinPageModule
        {
            get { return skinPageModule; }
            set { skinPageModule = value; }
        }
        ...
        private void GetModuleForSkinPageModule(int skinPageModule)
        {
            using (IDataReader reader = DBModule.GetModuleForSkinPageModule(skinPageModule))
            {
                PopulateFromReader(reader);
        ...
        public bool SaveForSkinPageModule()
        {
            if (this.moduleID > -1)
            {
                return Update();
            ...
            {
                return CreateForSkinPageModule();
            }
        }
        ...
        private bool CreateForSkinPageModule()
        {
            bool created = false;
            int newID = -1;
            ...
            newID = DBModule.AddModuleForSkinPageModule(
                    this.skinPageModule,
                    this.siteID,
                    this.siteGuid,
                    this.moduleDefID,
        ...
        public static List<Module> GetByPage(int pageId)
        {
            return LoadFromReader(DBModule.GetByPage(pageId));
        }

        public static List<Module> LoadFromReader(IDataReader reader)
        {
            List<Module> listModule = new List<Module>();
            while(reader.Read())
            {
                Module module = new Module();
                module.moduleID = Convert.ToInt32(reader["ModuleID"]);
                module.siteID = Convert.ToInt32(reader["SiteID"]);
                module.pageID = Convert.ToInt32(reader["PageID"]);
                module.moduleDefID = Convert.ToInt32(reader["ModuleDefID"]);
                module.moduleOrder = Convert.ToInt32(reader["ModuleOrder"]);
                module.paneName = reader["PaneName"].ToString();
                module.moduleTitle = reader["ModuleTitle"].ToString();
                module.authorizedEditRoles = reader["AuthorizedEditRoles"].ToString();
                module.draftEditRoles = reader["DraftEditRoles"].ToString();
                module.draftApprovalRoles = reader["DraftApprovalRoles"].ToString();
                module.cacheTime = Convert.ToInt32(reader["CacheTime"]);
                module.showTitle = Convert.ToBoolean(reader["ShowTitle"]);
                if (reader["EditUserID"] != DBNull.Value)
                {
                    module.editUserID = Convert.ToInt32(reader["EditUserID"]);
                }
                module.availableForMyPage = Convert.ToBoolean(reader["AvailableForMyPage"]);
                module.allowMultipleInstancesOnMyPage = Convert.ToBoolean(reader["AllowMultipleInstancesOnMyPage"]);
                if (reader["CreatedByUserID"] != DBNull.Value)
                {
                    module.createdByUserID = Convert.ToInt32(reader["CreatedByUserID"]);
                }
                if (reader["CreatedDate"] != DBNull.Value)
                {
                    module.createdDate = Convert.ToDateTime(reader["CreatedDate"]);
                }

                module.icon = reader["Icon"].ToString();


                module.guid = new Guid(reader["Guid"].ToString());
                module.featureGuid = new Guid(reader["FeatureGuid"].ToString());
                module.siteGuid = new Guid(reader["SiteGuid"].ToString());

                string edUserGuid = reader["EditUserGuid"].ToString();
                if (edUserGuid.Length == 36) module.editUserGuid = new Guid(edUserGuid);

                module.hideFromAuthenticated = Convert.ToBoolean(reader["HideFromAuth"]);
                module.hideFromUnauthenticated = Convert.ToBoolean(reader["HideFromUnAuth"]);

                module.viewRoles = reader["ViewRoles"].ToString();

                //ModuleDefinition moduleDef = new ModuleDefinition(module.moduleDefID);
                //module.controlSource = moduleDef.ControlSrc;
                //module.featureName = moduleDef.FeatureName;

                module.controlSource = reader["ControlSrc"].ToString();
                module.featureName = reader["FeatureName"].ToString();

                module.includeInSearch = Convert.ToBoolean(reader["IncludeInSearch"]);
                module.isGlobal = Convert.ToBoolean(reader["IsGlobal"]);
                module.headElement = reader["HeadElement"].ToString();

                module.publishMode = Convert.ToInt32(reader["PublishMode"]);

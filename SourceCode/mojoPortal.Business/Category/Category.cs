...
namespace mojoPortal.Business
{
    ...
    public class Category
    {
        ...
        public static List<int> GetAllCatagory(int SiteId)
        {
            List<int> result = new List<int>();
            using (var reader = DBCategory.GetAllCatagoryBySite(SiteId))
            {
                while (reader.Read())
                {
                    result.Add(Convert.ToInt32(reader["ItemID"]));
                }
            }
            return result;
        }

        public static List<CategorySite> GetStaticCatagory(string listCategory, DateTime? startDate, DateTime? endDate, int siteID)
        {
            List<CategorySite> result = new List<CategorySite>();
            using (var reader = DBCategory.GetAllCategory(listCategory, startDate, endDate, siteID))
            {
                while (reader.Read())
                {
                    CategorySite userStatic = new CategorySite();

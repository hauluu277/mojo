...
namespace mojoPortal.Business
{
    ...
    public class CommentRepository
    {
        ...
        public Comment Fetch(Guid guid)
        {
            using (IDataReader reader = DBComments.GetOne(guid))
            {
                if (reader.Read())
                {
                    ...
                    {
                        comment.FTS = reader["FTS"].ToString();
                    }
        ...
        public List<Comment> GetByContentDesc(Guid contentGuid,bool isAdmin)
        {
            IDataReader reader = DBComments.GetByContentDesc(contentGuid, isAdmin);
            return LoadListFromReader(reader);
        }
        ...
        public List<Comment> GetSelectPage(Guid siteGuid, Guid featureGuid,int published,int puApprove, int categoryID, Guid articleGuid,  int moderationStatus, DateTime? dateStart, DateTime? dateEnd, string keyword, int pageNumber, int pageSize, out int totalPages, out int totalCounts)
        {
            IDataReader reader = DBComments.GetSelectPage(siteGuid, featureGuid,published, puApprove, categoryID, articleGuid, moderationStatus, dateStart, dateEnd, keyword, pageNumber, pageSize, out totalPages, out totalCounts);
            return LoadCommentListFromReader(reader);
        }
        ///// <summary>
        ...
        //    return LoadListFromReader(reader);
        //}
        private List<Comment> LoadCommentListFromReader(IDataReader reader)
        {
            ...
            try
            {
                while (reader.Read())
                {
                    ...
                    if (!string.IsNullOrEmpty(reader["ArticleName"].ToString()))
                    {
                        comment.ArticleName = reader["ArticleName"].ToString();
                    }

                    if (!string.IsNullOrEmpty(reader["ArticleItemUrl"].ToString()))
                    {
                        comment.ArticleItemUrl = reader["ArticleItemUrl"].ToString();
                    }
                    if (!string.IsNullOrEmpty(reader["CategoryName"].ToString()))
                    {
                        comment.CategoryName = reader["CategoryName"].ToString();
                    }

                    if (!string.IsNullOrEmpty(reader["ArticleID"].ToString()))
                    {
                        comment.ArticleID = Convert.ToInt32(reader["ArticleID"]);
                    }
                    if (!string.IsNullOrEmpty(reader["ArticleStep"].ToString()))
                    {
                        comment.ArticleStep = Convert.ToInt32(reader["ArticleStep"]);
                    }

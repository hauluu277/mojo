...
namespace mojoPortal.Business
{
    ...
    public class Role
    {
        ...
        public static List<Role> GetUserRole(int userId)
        {
            var result = new List<Role>();
            using (IDataReader reader = DBRoles.GetRolesUser(userId))
            {
                while (reader.Read())
                {
                    var role = new Role();
                    role.UserID = Convert.ToInt32(reader["UserID"]);
                    role.FullName = reader["FullName"].ToString();
                    role.displayName = reader["RoleName"].ToString();
                    role.roleID = Convert.ToInt32(reader["RoleID"]);
                    role.roleGuid = Guid.Parse(reader["RoleGuid"].ToString());
                    result.Add(role);
                }
            }
            return result;

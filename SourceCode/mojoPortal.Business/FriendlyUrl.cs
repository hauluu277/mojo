...
namespace mojoPortal.Business
{
    ...
    public class FriendlyUrl
    {
        ...
        public static bool DeleteByPageGuid(Guid pageGuid, int siteId, string friendlyUrl)
        {
            if (pageGuid != Guid.NewGuid())
            {
                return DBFriendlyUrl.DeleteByPageGuid(pageGuid);
            }
            if (!string.IsNullOrEmpty(friendlyUrl))
            {
                friendlyUrl = friendlyUrl.Replace("~", string.Empty);
                if (friendlyUrl.StartsWith("/"))
                {
                    friendlyUrl = friendlyUrl.Substring(1);
                }
                var friend = new FriendlyUrl(siteId, friendlyUrl);
                if (friend != null)
                {
                    return DBFriendlyUrl.DeleteFriendlyUrl(friend.UrlId);
                }
            }
            return false;
        }
